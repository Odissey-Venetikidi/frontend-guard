...
```
Кладёт kit в `<проект>/.claude/frontend-guard/`, скил в `.claude/skills/frontend-guard/`, мержит
`.claude/settings.json`, ставит git pre-commit, добавляет `lint:ui` — **и сразу скаффолдит
CLAUDE-архитектуру** (всё под `.claude/`, см. ниже). Закоммить `.claude/` (внутри движок +
`guard.config.json`) — git-хук заработает у всех в команде. Старую корневую раскладку
(`.frontend-guard/`, корневые `guard.config.json`/`memory-bank/`) установщик сам переносит в `.claude/`.
...
```
your-project/
├ .mcp.json                 # ЕДИНСТВЕННЫЙ Claude-файл в корне (Claude Code требует тут)
├ tests/                    # тесты проекта (+ README с дисциплиной прогона)
└ .claude/                  # ВСЁ остальное, связанное с Claude
  ├ CLAUDE.md               #   правила проекта (.claude/CLAUDE.md авто-загружается)
  ├ CLAUDE.local.md         #   личные правки. в git не идёт (в .gitignore)
  ├ frontend-guard/         #   движок: guard.mjs · scaffold.mjs · гайд · hooks/
  ├ guard.config.json       #   конфиг движка (loadConfig ищет его тут)
  ├ memory-bank/            #   память: progress.md / decisions.md / insights.md
  ├ skills/                 #   навыки (1 папка + SKILL.md). + example-skill/
  ├ agents/                 #   суб-агенты, свой контекст. + example-agent.md
  ├ commands/               #   слэш-команды. + commit.md (/commit) + test.md (/test)
  ├ hooks/                  #   ограждения. + example-hook.sh + run-tests.sh (Stop: тесты)
  ├ plugins/                #   справочно (проектный .claude/plugins НЕ автозагружается)
  ...
  └ settings.example.json   #   полный образец (модель/права/хуки/Stop/statusLine)
```

В корне проекта — только `.mcp.json` (по требованию Claude Code) и `tests/`. Всё остальное под `.claude/`.
Папки `skills/agents/commands/hooks` приходят с README и рабочим примером — переименуй и наполни.
`plugins/` — справочная: проектная папка плагинов Claude Code **не** автозагружается (плагины ставятся
из маркетплейса / `~/.claude`). Активного поведения примеры не вносят: образцовый хук, `run-tests.sh`
и `statusLine` подключаются только если ты сам пропишешь их в `.claude/settings.json`.
...
## Конфиг (`.claude/guard.config.json`)
...
node guard.mjs --init       # написать .claude/guard.config.json из автоопределения
node guard.mjs --staged     # git-staged delta (зовётся pre-commit'ом)
node guard.mjs --lang flutter --json
node scaffold.mjs [dir]     # развернуть CLAUDE-архитектуру (всё под .claude/ + tests/)
```
